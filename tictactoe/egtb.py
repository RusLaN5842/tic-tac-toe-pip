...
class Generator:
    ...
    def save_results(self) -> str:
        ...
        with open(name, "wb") as file:
            for eight_bits in re.findall(r"\d{1,8}", "".join(self.results)):
                ...
                file.write(int.to_bytes(int(eight_bits, 2),
    ...
    def get_all_board(self) -> None:
        ...
        initial_board: npt.NDArray[np.int8] = np.array(
            [tictactoe.X] * number_of_x + [tictactoe.O] * number_of_o + [0] * empty_squares, dtype=np.int8)
        for permutation in set(itertools.permutations(initial_board)):
            numpy_board: npt.NDArray[np.int8] = np.array(
                permutation, dtype=np.int8).reshape(self.dimensions)
            ...
            fen = "".join(
                map(lambda piece: conversion_dict[piece], flattened_board))
            if has_x_won:
                result = f"{fen}{EGTB_X}"
...
class Reader:
    ...
    def read(self) -> None:
        ...
        with open(name, "rb") as file:
            ...
            while byte:
                ...
                byte = file.read(1)
        positions = [all_positions[i:i + number_of_bits]
                     for i in range(0, len(all_positions), number_of_bits)]
    ...
    def verify(self) -> None:
        ...
        if sha256_hash != self.verification_hash:
            logger.error(
                f"The calculated sha256 hash of {name} is {sha256_hash}, but the correct one is {self.verification_hash}.")
        else:
