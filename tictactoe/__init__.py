...
class Board:
    ...
    def find_directions(self) -> List[Tuple[int, ...]]:
        ...
        directions = list(itertools.product(
            [1, 0, -1], repeat=len(self.dimensions)))
        correct_directions = []
    ...
    def result(self) -> Optional[int]:
        ...
        if x_won and o_won:
            raise Exception(
                f"Both X and O have {self.x_in_a_row} pieces in a row.")
        elif x_won:
            return X
        elif o_won:
            return O
    ...
    def _get_dimension_repr(self, board_partition: npt.NDArray[np.int8]) -> str:
        ...
        if len(board_partition.shape) > 1:
            board_repr = ""
            divider = (
                (board_partition.shape[0] * 4 - 1) * "-" + "\n") * (len(board_partition.shape) - 1)
            for board_partition_index in range(board_partition.shape[-1]):
                board_repr += self._get_dimension_repr(
                    board_partition[..., board_partition_index]) + "\n"
                board_repr += divider
